...
import yaml
obs_info = {
    'dpix': 0.1, #pixel size in arcsec
    'bands': ['r'], #observation bands
    'zero_mag': [24], #the instrumental zero magnitude
    'zero_exp_time': 1.0, #the exposure time which defined the zero_mag
    'sky_mag': [22.35], #sky brightness, in mag/arcsec^2 unit
    'gains': [3.1],
    #the psf kernel are represented with a gaussian function,
    #you can set the width of psf, either via the ree80, sigma,
    #or fwhm of the gauss.
    ...
    #the full-width-half-maximum of the gaussian function, it is defined in 1d
    'psfs_fwhm': [0.18],
    'n_exp': 0, #number of exposure for y-band (not simulated here), csst have 4 exposures
    'exp_time': [1695], #total expsoure time
    'survey_area': 14500, #in deg^2 unit
    'readout': 4.5, #e-/pix
    'dark_current': 0.02, #e-/pix/s
    'rnd_obs': False,
    'rnd_obs_info': [None], 
